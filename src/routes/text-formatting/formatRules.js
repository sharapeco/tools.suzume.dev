...
import { normalize, stripFullwidthForm, stripJISX0201Kana, stripVerticalForms, toFullwidthForm } from "$lib/zenkaku";
...
export const formatRules = [
	...
	},
	{
		...
		fn(text, option) {
			switch (option) {
				case "normalize":
					return text.replace(/[\u{2F00}-\u{2FD5}]/gu, (match) => match.normalize("NFKC"));
			}
			return text;
		}
	},
	...
	},
	{
		...
		fn(text, option) {
			switch (option) {
				case "halfwidth":
					return text.replace(/[Ａ-Ｚａ-ｚ]+/gu, (match) => stripFullwidthForm(match));
				case "fullwidth":
	...
	},
	{
		...
		fn(text, option) {
			switch (option) {
				case "halfwidth":
					return text.replace(/[０-９]+/gu, (match) => stripFullwidthForm(match));
				case "fullwidth":
